...
import { TotalDurationSheet } from "./TotalDurationSheet"
...
export function KpiRow() {
  ...
  const metrics = [
    ...
    // Общий хронометраж теперь открывает окно выбора ТК
    { label: "Общий хронометраж", value: <button className="text-left text-xl font-bold text-foreground underline" onClick={() => setIsTotalOpen(true)}>{formatSeconds(kpi.totalDuration)}</button> },
        ...
        {metrics.map((m, i) => (
          <Card key={i} className="rounded-lg shadow-sm border border-border/60">
            <CardContent className="p-4 flex flex-col justify-center">
              ...
              <div className="text-xl font-bold text-foreground leading-none">{m.value}</div>
            </CardContent>
          </Card>
        ))}
      </div>

      <TotalDurationSheet open={isTotalOpen} onOpenChange={setIsTotalOpen} />
    </>
  )
}
